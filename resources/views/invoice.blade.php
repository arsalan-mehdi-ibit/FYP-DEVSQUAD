                    ...
                    <tbody>
                        @foreach ($invoices as $invoice)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="p-2 sm:p-3 text-xs text-left sm:text-sm md:text-base">{{ $loop->iteration }}</td>
                                <td class="p-2 sm:p-3 text-xs text-left sm:text-sm md:text-base">#INV{{ $invoice->id }}
                                </td>
                                <td class="p-2 sm:p-3 flex justify-left items-center">
                                    <img src="{{ asset('assets/profile.jpeg') }}" class="h-8 sm:h-10 rounded-full mr-2"
                                        alt="User">
                                    <span
                                        class="hidden sm:block text-xs sm:text-sm md:text-base">{{ $invoice->client->firstname ?? 'N/A' }}</span>
                                </td>
                                <td class="p-2 sm:p-3 text-xs text-left sm:text-sm md:text-base">
                                    ${{ number_format($invoice->amount, 2) }}</td>
                                <td class="p-2 sm:p-3 text-xs text-left sm:text-sm md:text-base">
                                    {{ $invoice->payment_method }}</td>
                                <td
                                    class="p-2 sm:p-3 text-xs text-left sm:text-sm md:text-base 
